...
import { ArrowLeft, Play, Plus, Trash2, Zap, AlertTriangle, Search, Settings2, Save, BarChart2, Clock, List, FileText, TrendingUp, TrendingDown } from 'lucide-react';
...
function BuilderContent() {
  ...
  const [activeTab, setActiveTab] = useState('overview'); // overview | audit | logs
  ...
  const IndicatorSelect = ({ side, data, onChange, onParamChange }: any) => {
    if (!data || !data.type) return <div className="text-red-500 text-xs">Invalid</div>;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 pb-20">
        ...
        <div className="col-span-3 space-y-4">
            ...
            {backtestResult && (
                <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} className="mt-8 bg-slate-900 rounded-2xl border border-slate-700 overflow-hidden">
                    <div className="flex border-b border-slate-700 bg-slate-950/50">
                        <button onClick={() => setActiveTab('overview')} className={'px-6 py-4 font-bold text-sm ' + (activeTab==='overview' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-500 hover:text-white')}>Overview</button>
                        <button onClick={() => setActiveTab('audit')} className={'px-6 py-4 font-bold text-sm ' + (activeTab==='audit' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-500 hover:text-white')}>Full Audit Report</button>
                        <button onClick={() => setActiveTab('logs')} className={'px-6 py-4 font-bold text-sm ' + (activeTab==='logs' ? 'text-emerald-400 border-b-2 border-emerald-400' : 'text-slate-500 hover:text-white')}>Trade Log</button>
                    </div>
                    ...
                    <div className="p-6">
                        ...
                        {activeTab === 'overview' && (
                            <>
                                ...
                                <div className="h-64 w-full bg-slate-950/50 rounded-xl border border-slate-800 p-2"><ResponsiveContainer width="100%" height="100%"><LineChart data={backtestResult.equity}><CartesianGrid strokeDasharray="3 3" stroke="#334155" /><XAxis dataKey="time" hide /><YAxis domain={['auto', 'auto']} stroke="#94a3b8" fontSize={10} /><Tooltip contentStyle={{backgroundColor: '#0f172a', border: '1px solid #334155'}} /><Legend /><Line name="Strategy" type="monotone" dataKey="balance" stroke="#3b82f6" strokeWidth={2} dot={false} /><Line name="Buy & Hold" type="monotone" dataKey="buy_hold" stroke="#eab308" strokeWidth={2} dot={false} /></LineChart></ResponsiveContainer></div>
                            </>
                        )}

                        {/* TAB 2: AUDIT */}
                        {activeTab === 'audit' && backtestResult.metrics.audit && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <h4 className="text-lg font-bold text-white mb-4">Risk Metrics</h4>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Max Drawdown</span><span className="text-red-400 font-bold">{backtestResult.metrics.audit.max_drawdown}%</span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Sharpe Ratio</span><span className="text-blue-400 font-bold">{backtestResult.metrics.audit.sharpe_ratio}</span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Profit Factor</span><span className="text-white font-bold">{backtestResult.metrics.audit.profit_factor}</span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Expectancy ($)</span><span className="text-emerald-400 font-bold"></span></div>
                                </div>
                                <div className="space-y-4">
                                    <h4 className="text-lg font-bold text-white mb-4">Consistency</h4>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Max Win Streak</span><span className="text-emerald-400 font-bold">{backtestResult.metrics.audit.max_win_streak}</span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Max Loss Streak</span><span className="text-red-400 font-bold">{backtestResult.metrics.audit.max_loss_streak}</span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Avg Win</span><span className="text-emerald-400 font-bold"></span></div>
                                    <div className="flex justify-between border-b border-slate-800 pb-2"><span className="text-slate-400">Avg Loss</span><span className="text-red-400 font-bold"></span></div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'logs' && (
                            <div className="max-h-96 overflow-y-auto">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-xs text-slate-500 uppercase bg-slate-950 sticky top-0"><tr><th className="px-6 py-3">Time</th><th className="px-6 py-3">Action</th><th className="px-6 py-3">Price</th><th className="px-6 py-3 text-right">PnL</th></tr></thead>
                                    <tbody>{backtestResult.trades.slice().reverse().map((t: any, i: number) => (<tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30"><td className="px-6 py-4 text-slate-400">{formatIST(t.entry_time)}</td><td className={'px-6 py-4 font-bold ' + (t.type.includes('BUY') ? 'text-emerald-400' : 'text-red-400')}>{t.type}</td><td className="px-6 py-4"></td><td className={'px-6 py-4 text-right font-bold ' + (t.pnl > 0 ? 'text-emerald-400' : 'text-red-400')}>{t.pnl > 0 ? '+' : ''}</td></tr>))}</tbody>
                                </table>
                            </div>
                        )}
                    </div>
