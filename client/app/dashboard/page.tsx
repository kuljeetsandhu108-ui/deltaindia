...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex relative font-sans">
      <aside className="w-72 border-r border-slate-800 p-8 hidden md:block bg-slate-950">
              ...
              <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} key={strat.id} className="p-6 rounded-2xl border border-slate-800 bg-slate-900 hover:border-indigo-500/50 transition-all group">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                    <Link href={'/dashboard/builder?edit=' + strat.id}><button className="p-2 rounded-lg bg-slate-800 hover:bg-white hover:text-black transition-all"><Edit3 size={18} /></button></Link>
                    <button onClick={() => handleDelete(strat.id)} className="p-2 rounded-lg bg-slate-800 hover:bg-red-500 hover:text-white transition-all"><Trash2 size={18} /></button>
                  </div>
                </div>
