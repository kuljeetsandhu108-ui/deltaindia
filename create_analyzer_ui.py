...
print("📊 Building Portfolio Analyzer UI...")

# A. Create the Analyzer Page
analyzer_code = """
"use client";
...
"""
os.system('mkdir -p client/app/dashboard/analyzer')
with open('client/app/dashboard/analyzer/page.tsx', 'w') as f: f.write(analyzer_code)
os.system('docker exec app-frontend-1 mkdir -p /app/app/dashboard/analyzer')
os.system('docker cp client/app/dashboard/analyzer/page.tsx app-frontend-1:/app/app/dashboard/analyzer/page.tsx')


# B. Add Link to Sidebar in Dashboard Layout
...
if 'Portfolio' not in d:
    new_btn = """<Link href="/dashboard/settings"><button className="w-full flex items-center gap-4 px-6 py-4 text-slate-400 hover:bg-slate-900 hover:text-white rounded-xl transition-all font-medium"><Database size={20} /> Data Sources</button></Link>
    ...
    d = d.replace('<Link href="/dashboard/settings"><button className="w-full flex items-center gap-4 px-6 py-4 text-slate-400 hover:bg-slate-900 hover:text-white rounded-xl transition-all font-medium"><Database size={20} /> Data Sources</button></Link>', new_btn)
    
    with open('./dashboard.tsx', 'w') as f: f.write(d)
    os.system('docker cp ./dashboard.tsx app-frontend-1:/app/app/dashboard/page.tsx')

print("✅ Analyzer Page Created & Linked!")
